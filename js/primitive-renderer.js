...
tplElementStatsDomain = (t, e) => {
    ...
    s.querySelector("td")
        .classList.add("stats"), s.classList.add("stats"), s.dataset.connectId = t.connectId, s.dataset.url = t.url, e.querySelector("tr.stats td.label")
        .querySelector(".border")
        .style.background = t.color;
    let r = e.querySelector("tr.stats td.content");
    return r.querySelector(".url")
        .setAttribute("href", "http://" + t.url), r.querySelector(".url")
        .textContent = t.url, r.querySelector(".rank-" + RANGE_TODAY + "-position")
        .textContent = t.ranks[RANGE_TODAY].position, r.querySelector(".rank-" + RANGE_TODAY + "-total")
        .textContent = t.ranks[RANGE_TODAY].total, r.querySelector(".rank-" + RANGE_ALLTIME + "-position")
        .textContent = t.ranks[RANGE_ALLTIME].position, r.querySelector(".rank-" + RANGE_ALLTIME + "-total")
        .textContent = t.ranks[RANGE_ALLTIME].total, e.querySelector(".time-min-date")
        .textContent = t.dates.timeMin, e.querySelector(".time-max-date")
        .textContent = t.dates.timeMax, e.querySelector(".time-min")
        .innerHTML = tplHtmlTimeObjectFragment({
            value: t.timeValues.min,
            resolution: RESOLUTION_HOURS
        }), e.querySelector(".time-max")
        .innerHTML = tplHtmlTimeObjectFragment({
            value: t.timeValues.max,
            resolution: RESOLUTION_HOURS
        }), r.querySelector(".days-domain")
        .textContent = t.days.domain, r.querySelector(".days-total")
        .textContent = t.days.total, r.querySelector(".visits-first")
        .textContent = t.visits.first, r.querySelector(".visits-last")
        .textContent = t.visits.last, r.querySelector(".time-" + RANGE_TODAY)
        .innerHTML = tplHtmlTimeObjectFragment({
            value: t.times[RANGE_TODAY],
            resolution: RESOLUTION_HOURS
        }), r.querySelector(".time-" + RANGE_AVERAGE)
        .innerHTML = tplHtmlTimeObjectFragment({
            value: t.times[RANGE_AVERAGE],
            resolution: RESOLUTION_HOURS
        }), r.querySelector(".time-" + RANGE_AVERAGE + "-pure")
        .innerHTML = tplHtmlTimeObjectFragment({
            value: t.times[RANGE_AVERAGE + "-pure"],
            resolution: RESOLUTION_HOURS
        }), r.querySelector(".time-" + RANGE_ALLTIME)
        .innerHTML = tplHtmlTimeObjectFragment({
            value: t.times[RANGE_ALLTIME],
            resolution: RESOLUTION_DAYS
        }), e
},
tplElementStatsOverall = (t, e) => (e.querySelector(".days-visited")
    .textContent = t.days.visited, e.querySelector(".days-total")
    .textContent = t.days.total, e.querySelector(".visits-first")
    .textContent = t.visits.first, e.querySelector(".visits-last")
    .textContent = t.visits.last, e.querySelector(".time-min-date")
    .textContent = t.dates.timeMin, e.querySelector(".time-max-date")
    .textContent = t.dates.timeMax, e.querySelector(".time-min")
    .innerHTML = tplHtmlTimeObjectFragment({
        value: t.timeValues.min,
    ...
    .innerHTML = tplHtmlTimeObjectFragment({
        value: t.timeValues.max,
        resolution: RESOLUTION_HOURS
    }), e.querySelector(".time-" + RANGE_TODAY)
    .innerHTML = tplHtmlTimeObjectFragment({
        value: t.times[RANGE_TODAY],
        resolution: RESOLUTION_HOURS
    }), e.querySelector(".time-" + RANGE_AVERAGE)
    .innerHTML = tplHtmlTimeObjectFragment({
        value: t.times[RANGE_AVERAGE],
        resolution: RESOLUTION_HOURS
    }), e.querySelector(".time-" + RANGE_AVERAGE + "-pure")
    .innerHTML = tplHtmlTimeObjectFragment({
        value: t.times[RANGE_AVERAGE + "-pure"],
        resolution: RESOLUTION_HOURS
    }), e.querySelector(".time-" + RANGE_ALLTIME)
    .innerHTML = tplHtmlTimeObjectFragment({
        value: t.times[RANGE_ALLTIME],
        resolution: RESOLUTION_DAYS
    }), e),
tplElementStatsCharts = (t, e) => {
    ...
    return s.querySelector(".info .left strong")
        .textContent = t.dates.start, s.querySelector(".info .right strong")
        .textContent = t.dates.today, e
},
