...
const getDomainsData = (e, t) => {
    ...
    updateDoughnutInfotext = (e, t, a) => {
        document.querySelector("#doughnut-" + e + " .foreign-object .name")
            .innerHTML = t, document.querySelector("#doughnut-" + e + " .foreign-object .percentage")
            .innerHTML = a
    },
...
const clearOverallStats = () => {
    document.querySelector("#pseudomodal .container.stats .stats-wrapper .text")
        .innerHTML = "", document.querySelector("#pseudomodal .container.stats .stats-wrapper .charts")
        .innerHTML = "", document.querySelector("#pseudomodal .container.stats .stats-wrapper")
        .dataset.statsReady = "0"
};
let renderUI = () => {
    renderUIRange(RANGE_TODAY, RESOLUTION_HOURS, !0, !0, !0), renderUIRange(RANGE_AVERAGE, RESOLUTION_HOURS, !0, !0, !1), renderUIRange(RANGE_ALLTIME, RESOLUTION_DAYS, !0, !0, !0), renderControlIdleTime(), renderControlGraphGap(), renderControlBadgeDisplay(), document.querySelector("#pseudomodal .container.stats .stats-wrapper .text")
        .innerHTML = "", document.querySelector("#pseudomodal .container.stats .stats-wrapper .charts")
        .innerHTML = "", document.querySelector("#pseudomodal .container.stats .stats-wrapper")
        .dataset.statsReady = "0", dcl("UI rendered")
};
const screenshotUIShow = () => {
        document.querySelector("html")
            .classList.add("screenshot-mode"), document.querySelector("footer")
            .style.display = "none", screenshotUICaptureShow(), chrome.permissions.contains({
                origins: ["<all_urls>"]
            }, e => {
                if (chrome.runtime.lastError && chrome.runtime.lastError.message && dcl("Screenshot - permission.contains: " + chrome.runtime.lastError.message), e) {
                    if (!backgroundJS.settings.screenshotInstructionsRead) {
                        ...
                        elementInsertIntoElement(document.querySelector(".screenshot-info .instructions .list"), t), document.querySelector(".screenshot-overlay")
                            .style.display = "block", document.querySelector(".screenshot-info")
                            .style.display = "block", document.querySelector(".screenshot-info .instructions")
                            .style.display = "block", document.querySelector(".screenshot-info .permission-grant")
                            .style.display = "none", backgroundJS.setScreenshotInstructionsRead(!0), backgroundJS.saveScreenshotInstructionsRead()
                    }
                } else document.querySelector(".screenshot-overlay")
                    .style.display = "block", document.querySelector(".screenshot-info")
                    .style.display = "block", document.querySelector(".screenshot-info .instructions")
                    .style.display = "none", document.querySelector(".screenshot-info .permission-grant")
                    .style.display = "block"
            })
    },
    screenshotUIHide = () => {
        document.querySelector(".screenshot-overlay")
            .style.display = "none", document.querySelector(".screenshot-info")
            .style.display = "none"
    },
...
renderUI(), window.location.search === SCREENSHOT_MODE_QUERY && (SCREENSHOT_MODE = !0, screenshotUIShow()), dcl("Application initialized"), addMultipleDelegatedEventListeners("body", "click", (e, t) => {
    ...
    let a = t.closest(".chart-days")
        .querySelector(".info .date"),
        n = t.closest(".chart-days")
    ...
    let a = t.closest(".chart-daynames")
        .querySelector(".info .percentage"),
        n = t.closest(".chart-daynames")
        .querySelector(".info .time"),
    ...
    document.querySelector("#pseudomodal #idle-time .display")
        .innerHTML = s, "change" === e.type && (backgroundJS.setIdleTime(n), backgroundJS.saveIdleTime(), dcl("Idle time saved: " + n))
    ...
    document.querySelector("#pseudomodal #graph-gap .display")
        .innerHTML = a, "change" === e.type && (backgroundJS.setGraphGap(n), backgroundJS.saveGraphGap(), renderUIRange(RANGE_TODAY, RESOLUTION_HOURS, !0, !1, !1), renderUIRange(RANGE_AVERAGE, RESOLUTION_DAYS, !0, !1, !1), renderUIRange(RANGE_ALLTIME, RESOLUTION_DAYS, !0, !1, !1), dcl("Graph gap saved: " + n))
    ...
    e.preventDefault(), t.parentNode.querySelector(".confirm")
    ...
    e.preventDefault(), t.classList.contains("active") ? (t.classList.remove("active"), a.innerText = a.dataset.default, n.classList.remove("running"), backgroundJS.clearAllGeneratedData(), renderUI(), dcl("All data cleared")) : (t.classList.add("active"), a.innerText = a.dataset.active, n.classList.add("running", "warning"), n.querySelector(".shifter")
        .style.animationDuration = INTERVAL_UI_LOADING + "ms", setTimeout(() => {
    ...
    e.preventDefault(), console.log("Screenshot - start"), document.querySelector(".screenshot-capture")
...
const restoreFromJson = e => {
        let t = document.querySelector("#pseudomodal .options-restore"),
            ...
            n = t.querySelector(".loading");
        const s = () => {
            a.innerText = a.dataset.warning, n.classList.add("blinking", "warning"), n.querySelector(".shifter")
                .style.animationDuration = UI_LOADING_BLINKING_INTERVAL + "ms", n.querySelector(".shifter")
                .style.animationIterationCount = UI_LOADING_BLINKING_COUNT, setTimeout(() => {
    ...
    renderDomainStats = e => {
        ...
        if ("1" !== e.contextEl.dataset.statsReady) {
            ...
            a = Object.assign(a, e), a.color = e.contextEl.querySelector(".label span")
                .style.color;
        ...
        "1" === e.contextEl.dataset.statsVisible ? (document.querySelector("#table-" + e.range + ' tr.stats[data-url="' + e.url + '"]')
            .style.display = "none", e.contextEl.dataset.statsVisible = "0") : (document.querySelector("#table-" + e.range + ' tr.stats[data-url="' + e.url + '"]')
            .style.display = "table-row", e.contextEl.dataset.statsVisible = "1")
    },
    getDomainData = e => {
        ...
        for (o = getDatesSparse(backgroundJS.dates.start, h - 1), l = [0, 0, 0, 0, 0, 0, 0], t = 0; t < o.length; t++) s = o[t], r = 0, y.days.hasOwnProperty(o[t]) && (S++, r = y.days[s].seconds, c = r < d ? s : c, u = r > i ? s : u, d = r < d ? r : d, i = r > i ? r : i, m = s < m ? s : m, g = s > g ? s : g), n = (new Date(s)
            .getDay() + 6) % 7, l[n] += r, p.push({
            date: s,
            seconds: r
        });
        return a = {
    ...
    getOverallData = () => {
        ...
        for (e = 0; e < d.length; e++) n = d[e], r = 0, s.hasOwnProperty(d[e]) && (r = s[n].seconds, m = r < c ? n : m, g = r > u ? n : g, c = r < c ? r : c, u = r > u ? r : u, p = n < p ? n : p, S = n > S ? n : S), a = (new Date(n)
            .getDay() + 6) % 7, i[a] += r, r > 0 && h++, y.push({
            date: n,
            seconds: r
        });
        return t = {
